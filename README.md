...
ブラウザで動く Web Audio APIベースのルーパーアプリ。即興的にループを重ねていくのに特化したUIデザインで、ライブパフォーマンスやアイデアスケッチに適した設計となっている。
...
- **ストレッチ** — BPMに合わせて音程を変えずにフレーズの長さを自動調整
...
本アプリはAI学習のためにユーザーの音声データを収集することはありません。録音された音声データはブラウザのメモリ上でのみ処理され、サーバへ送信することやローカルファイルに保存することはありません。
...
├── Transport.js   # BPMクロック・スケジューラー
├── LoopTrack.js   # ループトラック・波形
├── Recorder.js    # 録音機能
├── Metronome.js   # メトロノーム
├── Synth.js       # 内蔵シンセサイザー
├── TimeStretch.js # ストレッチ処理
└── UI.js          # ユーザインタフェース
```
