...
class LoopScheduler {
  ...
  _scheduleOnce(startTime) {
    ...
    // stopAt=audio context time to stop.
    // Adjusts bufOffset for late scheduling so audio stays in sync.
    const place = (when, bufOffset, stopAt) => {
    ...
    // TIMING=N shifts the audio start N beats later than the bar head.
    // Buffer always plays from offset 0, head→tail, for min(bufDur, span) seconds.
    const timingNorm   = ((timing % spanBeats) + spanBeats) % spanBeats;
    const timingOffset = timingNorm * beatDur;
    const S_adj        = S + timingOffset;
    if (S_adj < now - 0.05) return;
    place(S_adj, 0, S_adj + Math.min(bufDur, span));
  }
